...
log_path = r'C:\Solar-Flare-Forecast\Logging\trainig_tuning.log'
...
logging.info('Pipeline Building and Hyperparametr turning have started')

class auto_pipeline(BaseEstimator):

    def __init__(self, df:pd.DataFrame, target:str, model, model_name, params):
        self.df = df.copy()
        self.target = target
    ...
    def tuning(self):
        logging.info("HYPERPARAMETR TUNING STARTED")
        ...
        try:
            ...
            grid_search = GridSearchCV(self.model_algorithm, self.params, cv=5, n_jobs=-1, verbose=2)
            grid_search.fit(self.X_train, self.y_train)
            self.models[self.model_name] = grid_search.best_estimator_
            # print(f"Best parameters for {self.model_name}: {grid_search.best_params_}")
            logging.info('Hyperparametr tuning is DONE')
            return self
        except Exception as e:
            logging.error(f'ERROR while tuning:{e}')
            raise
    ...
    def model_saving(self):
        out_dir = r'C:\Solar-Flare-Forecast\Models\All_Models'
    ...
    def metrics_saving(self):
        try:
            ...
            #output path
            output_dir = r'C:\Solar-Flare-Forecast\Metrics'

            os.makedirs(output_dir, exist_ok=True)
            output_file = os.path.join(output_dir, f'{self.model_name}_metrics.csv')
            # DataFrame to CSV
            df.to_csv(output_file, index=False)
            logging.info(f'Metrics are savded at {r'C:\Solar-Flare-Forecast\Metrics'}')
            return self
        except Exception as e:
